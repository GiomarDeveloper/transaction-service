...
/**
 * Consumidor de eventos Kafka para transacciones.
 * Escucha y procesa eventos de pagos, consumos y retiros bancarios.
 */
@Slf4j
@Component
...
public class TransactionEventConsumer {
  ...
  /**
   * Consume eventos de solicitud de pago de transacciones.
   *
   * @param message el mensaje Kafka en formato JSON
   */
  ...
  public void consumeTransactionPaymentRequest(String message) {
    try {
      ...
      TransactionPaymentRequestEvent event =
        objectMapper.readValue(message, TransactionPaymentRequestEvent.class);
  ...
  /**
   * Consume eventos de solicitud de consumo de transacciones.
   *
   * @param message el mensaje Kafka en formato JSON
   */
  ...
  public void consumeTransactionConsumptionRequest(String message) {
    try {
      ...
      TransactionConsumptionRequestEvent event =
        objectMapper.readValue(message, TransactionConsumptionRequestEvent.class);
      log.info(
        "🎯 Received transaction consumption request - CreditId: {}, Amount: {}, Merchant: {}",
  ...
  /**
   * Consume eventos de solicitud de retiro bancario.
   *
   * @param message el mensaje Kafka en formato JSON
   */
  ...
  public void consumeBankWithdrawalRequest(String message) {
    try {
      ...
      BankWithdrawalRequestEvent event =
        objectMapper.readValue(message, BankWithdrawalRequestEvent.class);
      log.info(
        "🎯 Parsed bank withdrawal request - WithdrawalId: {}, RequestId: {}, Account: {}, Amount: {}",
