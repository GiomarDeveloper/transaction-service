 ...
 * Implementa la API generada por OpenAPI para el manejo de transacciones.
 */
@Slf4j
@RestController
...
public class TransactionController implements TransactionsApi {
  ...
  @Override
  public Mono<ResponseEntity<Flux<TransactionResponse>>> getAll(String customerId,
                                                                ProductTypeEnum productType,
                                                                String productId,
                                                                LocalDate startDate,
                                                                LocalDate endDate,
                                                                ServerWebExchange exchange) {
    log.info("GET /transactions - customerId: {}, productType: {}, productId: {}", customerId,
  ...
  @Override
  public Mono<ResponseEntity<Flux<TransactionResponse>>> getByAccount(String accountId,
                                                                      ServerWebExchange exchange) {
    log.info("GET /transactions/account/{}", accountId);

    Flux<TransactionResponse> transactions =
      transactionInputPort.getTransactionsByAccount(accountId)
        .map(transactionApiMapper::toResponse);
  ...
  @Override
  public Mono<ResponseEntity<Flux<TransactionResponse>>> getByCredit(String creditId,
                                                                     ServerWebExchange exchange) {
  ...
  @Override
  public Mono<ResponseEntity<Flux<TransactionResponse>>> getByCustomer(String customerId,
                                                                       ServerWebExchange exchange) {
    log.info("GET /transactions/customer/{}", customerId);

    Flux<TransactionResponse> transactions =
      transactionInputPort.getTransactionsByCustomer(customerId)
        .map(transactionApiMapper::toResponse);
  ...
  @Override
  public Mono<ResponseEntity<TransactionResponse>> makeDeposit(Mono<DepositRequest> depositRequest,
                                                               ServerWebExchange exchange) {
    return depositRequest.flatMap(request -> {
      log.info("POST /transactions/deposit - accountId: {}, amount: {}", request.getAccountId(),
        request.getAmount());

      return transactionInputPort.makeDeposit(request.getAccountId(), request.getAmount(),
  ...
  @Override
  public Mono<ResponseEntity<TransactionResponse>> makeWithdrawal(
    Mono<WithdrawalRequest> withdrawalRequest, ServerWebExchange exchange) {
    return withdrawalRequest.flatMap(request -> {
      log.info("POST /transactions/withdrawal - accountId: {}, amount: {}", request.getAccountId(),
        request.getAmount());

      return transactionInputPort.makeWithdrawal(request.getAccountId(), request.getAmount(),
          request.getDescription())
        .map(transactionApiMapper::toResponse)
        .map(response -> ResponseEntity.status(201).body(response));
    });
  ...
  @Override
  public Mono<ResponseEntity<TransactionResponse>> makeCreditConsumption(
  ...
  @Override
  public Mono<ResponseEntity<TransactionResponse>> makeCreditPayment(
    Mono<PaymentRequest> paymentRequest, ServerWebExchange exchange) {
    return paymentRequest.flatMap(request -> {
      log.info("POST /transactions/credit-payment - creditId: {}, amount: {}",
        request.getCreditId(), request.getAmount());

      return transactionInputPort.makeCreditPayment(request.getCreditId(), request.getAmount(),
          request.getDescription())
        .map(transactionApiMapper::toResponse)
        .map(response -> ResponseEntity.status(201).body(response));
    });
  ...
  @Override
  public Mono<ResponseEntity<Flux<TransactionResponse>>> makeTransfer(
  ...
  @Override
  public Mono<ResponseEntity<Flux<TransactionResponse>>> getProductTransactionsForCurrentMonth(
    String productId, String productType, ServerWebExchange exchange) {
  ...
  @Override
  public Mono<ResponseEntity<Flux<CommissionReport>>> getCommissionsReport(LocalDate startDate,
                                                                           LocalDate endDate,
                                                                           String productType,
                                                                           ServerWebExchange exchange) {
  ...
  @Override
  public Mono<ResponseEntity<TransactionResponse>> makeThirdPartyCreditPayment(
    Mono<ThirdPartyCreditPaymentRequest> thirdPartyCreditPaymentRequest,
    ServerWebExchange exchange) {
  ...
  @Override
  public Mono<ResponseEntity<LastMovementsResponse>> getLast10Movements(String productId,
                                                                        ProductTypeEnum productType,
                                                                        ServerWebExchange exchange) {
